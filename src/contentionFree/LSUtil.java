...
public class LSUtil {
	...
	public LSUtil(Workflow wf) {
		...
		Collections.sort(tasks, new TProperties(wf, TProperties.Type.GAMMA)); //sort based on bLevel
...
//				solution.addTaskToVMEnd(selectedVM, tasks.get(0), minEST);
	...
	public static List<VM> buildVMList(final Solution solution) {
		boolean emptyVMFlag = false;	//始终保持一个new vm,是否指的就是空的vm
		...
		Collections.sort(vmList, new Comparator<VM>(){//空的放后面
