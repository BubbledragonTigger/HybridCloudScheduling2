...
import contentionAware.CSolution;
import contentionAware.EAllocation;
...
public class ChartUtil {
	...
	private static boolean exportPNGFlag = true;
	/**
	...
	public static void visualizeSchedule(Solution s) {
	    ...
	    for(VM vm : s.getUsedVMSet()){
	    	int id = vm.getId();
	    	int type = vm.getType();
	    	vms[id] = "v" + turnIntToLowerString(id + 1);	//+"_"+type
	    }
	    ...
	    renderer.setSeriesPaint(0,new Color(255, 150, 150));
	    renderer.setSeriesPaint(1,new Color(200, 200, 255));
	    renderer.setSeriesPaint(2,new Color(150, 255, 150));
...
//	    plot.getRangeAxis().setRange(0, 70);		//仅用于画示例的
	    ...
	    if(exportPNGFlag)
		    try{
		    	...
		    	ChartUtilities.writeScaledChartAsPNG(os, chart, 1000, 625, scale, scale);
		    	os.close();
		    }catch(IOException e){}	    
	    if(panel==null){
	...
	private static String turnIntToLowerString(int id){
		String s = "";
		do{
			int t = id % 10;
			char c = (char)(t + SZERO);
