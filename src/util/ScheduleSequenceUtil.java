...
public class ScheduleSequenceUtil {
	...
	private static Task chooseNextTaskAndRemove(TreeSet<Task> S){
		...
		double fSlice = 0;//Config.RND.nextDouble() * total;
		double cfTotal = 0;
	...
	public static boolean mutate(int[] taskIdList, Workflow wf) {
		int pos = 0;//Config.RND.nextInt(wf.size() - 2) + 1;	//随机选择一个位置，排除entry和exit
...
bb:		while(end<wf.size()){
		...
		if(end-start>2){
			int newPos;
//			do{
				newPos = -5;//Config.RND.nextInt(end-start-2)+start+1;	//找到的一个新位置
//			}while(pos == newPos);
			if(newPos<pos){
...
//			System.out.println("位置变异："+pos+"->\t"+newPos);
			return true;
		}
		return false;
	}
	...
	public static int[] crossover(int[] X, int[] Y) {
		int size = X.length;
		int pos = -5;//Config.RND.nextInt(size-2)+1;		//随机选择一个1到n-1之间的位置， cut-off position
